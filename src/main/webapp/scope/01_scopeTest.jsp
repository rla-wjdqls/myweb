...
----------------------------------------------------------------------------------
	...
	● [Scope의 종류]
	  ...
	  - 1)~4) 순서대로 체크해야 한다
	  1)pageContext (내 자리)  : 현재 페이지에서만 유효. 기본값
	  2)request     (내 캐비넷) : 부모페이지(호출함)와 자식페이지(호출당함)에서만 유효
	  3)session				 : myweb 프로젝트의 모든 페이지에서 유효 (사용자 개별 접근, 시간)
	  4)application (강의장)   : myweb 프로젝트의 모든 페이지에서 유효 (모든 사용자 접근, 서버 정보)
...
*/
	...
	int kor = (int)obj;							  //형변환
	int eng = (int)request.getAttribute("eng");   //바로 형변환해서 가져옴
